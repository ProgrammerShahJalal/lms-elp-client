...
import { useAddCourseMutation, useGetAllCoursesQuery } from '@/redux/api/courseApi';
...
const AddCourseForm = () => {
    ...
    const { data: courses, isLoading: isSubcategoryLoading } = useGetAllCoursesQuery();
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <h1 className="text-2xl font-bold mb-4 mt-12">Admin Update & Delete Courses</h1>
                        ...
                        <tbody>
                            {allCourses?.map((course) => (
                                <tr key={course._id}>
                                    ...
                                    <td className="py-2 px-4 border-b">{course?.membership_type}</td>
                                    <td className="py-2 px-4 border-b">{course?.category_id?.title}</td>
                                    <td className="py-2 px-4 border-b">
                                        <Image src={course.banner} alt={`Banner for ${course.title}`} width={400} height={300} />
                                    </td>
                                    <td className="py-2 px-4 border-b md:table-cell">
                                        <button className="bg-blue-500 text-white py-1 px-2 rounded-md">Update</button>
                                    </td>
                                    <td className="py-2 px-4 border-b md:table-cell">
                                        <button className="bg-red-500 text-white py-1 px-2 rounded-md" onClick={() => handleDelete(course.id)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
