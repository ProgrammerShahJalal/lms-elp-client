  ...
  return (
    <>
      <div className="flex gap-5 lg:mx-14 items-center sm:mt-14 lg:mt-24 ">
        ...
        <button onClick={() => setShowCourses(true)} className=" text-bluePrimary hover:text-greenPrimary    rounded transition-all duration-500 delay-200 font-bold lg:text-xl ">
          সব  কোর্স দেখুন
        </button>
        <button onClick={() => setShowCourses(false)} className="  text-bluePrimary hover:text-greenPrimary    rounded transition-all duration-500 delay-200 font-bold lg:text-xl">
          সব বই দেখুন
        </button>
      </div>
      {showCourses ? <SubCategoryCourses sub_category_id={sub_category_id} /> : <SubCategoryBooks sub_category_id={sub_category_id} />}
      <div className="flex justify-end">
        <Link href={`/courses/category/subcategory/courseBundle/${sub_category_id}`} className=" bg-bluePrimary text-white hover:bg-greenPrimary    rounded transition-all duration-500 delay-200 font-bold text-xl border px-10 py-3 lg:mb-0 mb-5">
